...
ChatLogic::ChatLogic()
{
    std::cout << "ChatLogic " << this << " constructed " << std::endl;
}

ChatLogic::~ChatLogic()
{
    std::cout << "ChatLogic " << this << " destructed " << std::endl;
}
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // the lambda function for the predicate must be a reference, otherwise the unique ptr would be copied
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> &node) { return node->GetID() == id; });
                        // minimal example about tdiffernce from  std::unique_ptr<GraphNode> &node to std::unique_ptr<GraphNode> node
    ...
    _chatBot = &chatBot; // chatlogic uses this as a communication handle
...
{
    std::cout << chatbot << " (ChatBot handle) set on " << this << " (ChatLogic)" << std::endl;
